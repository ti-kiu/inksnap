...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `Ideias de Tatuagem ${name} — Inspiração e Prévia Grátis | InkPreview`,
    description: `Descubra ideias de tatuagem ${name} com inspiração IA. Faça uma prévia grátis no seu corpo com o InkPreview.`,
    alternates: {
    ...
    openGraph: {
      title: `Ideias de Tatuagem ${name} | InkPreview`,
      description: `Descubra ideias de tatuagem ${name} com inspiração IA.`,
...
export default async function PtPseoStylePage({ params }: PageProps) {
  const { slug } = await params;
  const page = content[slug as keyof typeof content];
